...
These prompt recipes are for the Pi `imagegen` tool.
...
For transparent images, prompt for a flat chroma-key background and remove it locally with `python "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/imagegen/scripts/remove_chroma_key.py"`.
...
Post-process note: after Pi imagegen generation, run `python "${PI_CODING_AGENT_DIR:-$HOME/.pi/agent}/skills/imagegen/scripts/remove_chroma_key.py" --input <source> --out <final.png> --auto-key border --soft-matte --transparent-threshold 12 --opaque-threshold 220 --despill`. For true/native transparency requests, failed chroma-key validation, or complex subjects such as hair, fur, glass, smoke, liquids, translucent materials, reflections, or soft shadows, explain the chroma-key limitation and ask how the user wants to proceed.

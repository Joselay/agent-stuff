...
function tmuxSocketPath(): string {
	return path.join(subagentCacheDir(), "tmux.sock");
}

function subagentCacheDir(): string {
	const cacheRoot = process.env.XDG_CACHE_HOME || path.join(homedir(), ".cache");
	return path.join(cacheRoot, "pi", RUNS_DIR);
}
...
export default function subagentExtension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			const runDir = path.join(subagentCacheDir(), ctx.sessionManager.getSessionId(), childSessionId);
